...
public abstract class Pack {
    ...
    public static short bigEndianToShort(byte[] arrby, int n) {
    ...
    public static void intToBigEndian(int[] arrn, byte[] arrby, int n) {
        ...
        for (n = n2; n < arrn.length; ++n) {
            Pack.intToBigEndian(arrn[n], arrby, n3);
            n3 += 4;
        }
    }
    ...
    public static int[] littleEndianToInt(byte[] arrby, int n, int n2) {
        int[] arrn = new int[n2];
        int n3 = 0;
        n2 = n;
        for (n = n3; n < arrn.length; ++n) {
            arrn[n] = Pack.littleEndianToInt(arrby, n2);
            n2 += 4;
        }
        return arrn;
    }
    ...
    public static void littleEndianToLong(byte[] arrby, int n, long[] arrl, int n2, int n3) {
        int n4 = 0;
        int n5 = n;
        for (n = n4; n < n3; ++n) {
            arrl[n2 + n] = Pack.littleEndianToLong(arrby, n5);
            n5 += 8;
        }
    }

    public static short littleEndianToShort(byte[] arrby, int n) {
        return (short)(arrby[n] & 255 | (arrby[n + 1] & 255) << 8);
    }
    ...
    public static void longToBigEndian(long[] arrl, byte[] arrby, int n) {
        ...
        for (n = n2; n < arrl.length; ++n) {
            Pack.longToBigEndian(arrl[n], arrby, n3);
            n3 += 8;
        }
    }
    ...
    public static void longToLittleEndian(long[] arrl, int n, int n2, byte[] arrby, int n3) {
        for (int i = 0; i < n2; ++i) {
            Pack.longToLittleEndian(arrl[n + i], arrby, n3);
            n3 += 8;
        }
    }
    ...
    public static void shortToLittleEndian(short s, byte[] arrby, int n) {
        arrby[n] = (byte)s;
        arrby[n + 1] = (byte)(s >>> 8);
    }
    ...
    public static byte[] shortToLittleEndian(short s) {
        ...
        Pack.shortToLittleEndian(s, arrby, 0);
        return arrby;
    }
}
