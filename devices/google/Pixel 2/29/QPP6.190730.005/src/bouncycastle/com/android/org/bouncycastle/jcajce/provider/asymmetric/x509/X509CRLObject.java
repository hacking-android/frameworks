...
import com.android.org.bouncycastle.jcajce.provider.asymmetric.x509.ExtCRLException;
import com.android.org.bouncycastle.jcajce.provider.asymmetric.x509.X509CRLEntryObject;
...
extends X509CRL {
    ...
    protected X509CRLObject(JcaJceHelper jcaJceHelper, CertificateList object) throws CRLException {
    ...
    private void doVerify(PublicKey serializable, Signature signature) throws CRLException, NoSuchAlgorithmException, InvalidKeyException, SignatureException {
        if (this.c.getSignatureAlgorithm().equals(this.c.getTBSCertList().getSignature())) {
            byte[] arrby = this.sigAlgParams;
            if (arrby != null) {
                try {
                    X509SignatureUtil.setSignatureParameters(signature, ASN1Primitive.fromByteArray(arrby));
                }
                catch (IOException iOException) {
                    serializable = new StringBuilder();
                    ((StringBuilder)serializable).append("cannot decode signature parameters: ");
                    ((StringBuilder)serializable).append(iOException.getMessage());
                    throw new SignatureException(((StringBuilder)serializable).toString());
                }
            }
    ...
    static boolean isIndirectCRL(X509CRL arrby) throws CRLException {
        boolean bl;
        block4 : {
            block3 : {
                try {
    ...
    public boolean isRevoked(Certificate object) {
        if (!((Certificate)object).getType().equals("X.509")) throw new IllegalArgumentException("X.509 CRL used with non X.509 Cert");
        Enumeration enumeration = this.c.getRevokedCertificateEnumeration();
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (cRLEntry.getUserCertificate().getValue().equals(bigInteger)) {
                ...
                return false;
                    ...
                    object = new StringBuilder();
                    ((StringBuilder)object).append("Cannot process certificate: ");
                    ((StringBuilder)object).append(certificateEncodingException.getMessage());
    ...
    public String toString() {
        ...
        if (extensions != null) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                Object object2 = extensions.getExtension((ASN1ObjectIdentifier)object);
