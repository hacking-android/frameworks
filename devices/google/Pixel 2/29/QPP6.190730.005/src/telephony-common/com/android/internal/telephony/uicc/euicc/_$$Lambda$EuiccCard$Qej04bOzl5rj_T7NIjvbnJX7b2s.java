/*
 * Decompiled with CFR 0.145.
 * 
 * Could not load the following classes:
 *  com.android.internal.telephony.uicc.euicc.-$
 *  com.android.internal.telephony.uicc.euicc.-$$Lambda
 *  com.android.internal.telephony.uicc.euicc.-$$Lambda$EuiccCard
 *  com.android.internal.telephony.uicc.euicc.-$$Lambda$EuiccCard$Qej04bOzl5rj_T7NIjvbnJX7b2s
 */
package com.android.internal.telephony.uicc.euicc;

import com.android.internal.telephony.uicc.euicc.-$;
import com.android.internal.telephony.uicc.euicc.EuiccCard;

public final class _$$Lambda$EuiccCard$Qej04bOzl5rj_T7NIjvbnJX7b2s
implements EuiccCard.ApduResponseHandler {
    public static final /* synthetic */ -$.Lambda.EuiccCard.Qej04bOzl5rj_T7NIjvbnJX7b2s INSTANCE;

    static /* synthetic */ {
        INSTANCE = new _$$Lambda$EuiccCard$Qej04bOzl5rj_T7NIjvbnJX7b2s();
    }

    private /* synthetic */ _$$Lambda$EuiccCard$Qej04bOzl5rj_T7NIjvbnJX7b2s() {
    }

    public final Object handleResult(byte[] arrby) {
        return EuiccCard.lambda$getDefaultSmdpAddress$19(arrby);
    }
}

