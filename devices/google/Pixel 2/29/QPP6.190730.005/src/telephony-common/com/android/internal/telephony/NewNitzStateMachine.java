...
implements NitzStateMachine {
    ...
    private void updateTimeFromNitz() {
        TimestampedValue<NitzData> timestampedValue;
        block10 : {
            timestampedValue = this.mLatestNitzSignal;
            if (!this.mDeviceState.getIgnoreNitz()) break block10;
            Rlog.d((String)LOG_TAG, (String)"updateTimeFromNitz: Not suggesting system clock because gsm.ignore-nitz is set");
            return;
        }
        this.mWakeLock.acquire();
        long l = this.mTimeServiceHelper.elapsedRealtime();
        long l2 = timestampedValue.getReferenceTimeMillis();
        {
        ...
        if (l2 >= 0L && l2 <= Integer.MAX_VALUE) {
            this.mWakeLock.release();
            Object object = new TimestampedValue(timestampedValue.getReferenceTimeMillis(), (Object)((NitzData)timestampedValue.getValue()).getCurrentTimeInMillis());
            if (this.mSavedNitzTime != null) {
                int n = this.mDeviceState.getNitzUpdateSpacingMillis();
                int n2 = this.mDeviceState.getNitzUpdateDiffMillis();
                l = object.getReferenceTimeMillis() - this.mSavedNitzTime.getReferenceTimeMillis();
                l2 = (Long)object.getValue();
                long l3 = (Long)this.mSavedNitzTime.getValue();
                if (l <= (long)n && Math.abs(l2 - l3 - l) <= (long)n2) {
                    object = new StringBuilder();
                    ((StringBuilder)object).append("updateTimeFromNitz: not setting time. NITZ signal is too similar to previous value received  mSavedNitzTime=");
                    ((StringBuilder)object).append(this.mSavedNitzTime);
                    ((StringBuilder)object).append(", nitzSignal=");
                    ((StringBuilder)object).append(timestampedValue);
                    ((StringBuilder)object).append(", nitzUpdateSpacing=");
                    ((StringBuilder)object).append(n);
                    ((StringBuilder)object).append(", nitzUpdateDiff=");
            ...
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("updateTimeFromNitz: suggesting system clock update nitzSignal=");
            stringBuilder.append(timestampedValue);
            stringBuilder.append(", newNitzTime=");
            ...
            this.mTimeServiceHelper.suggestDeviceTime((TimestampedValue<Long>)object);
            TelephonyMetrics.getInstance().writeNITZEvent(this.mPhone.getPhoneId(), (Long)object.getValue());
            this.mSavedNitzTime = object;
            return;
        }
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("updateTimeFromNitz: not setting time, unexpected elapsedRealtime=");
        ...
        try {
            ...
            stringBuilder = new StringBuilder();
            stringBuilder.append("updateTimeFromNitz: Processing NITZ data nitzSignal=");
            stringBuilder.append(timestampedValue);
